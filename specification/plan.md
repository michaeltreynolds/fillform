...
- Parent birth date = approximate single year `about <childYear − offset>`,
  `offset` = 21 (mother) / 25 (father) — a search seed, not a known date.
...
| Status | **Deceased** |
| Birth Date | Approximate single year `about <childYear − offset>`, where `offset` = 21 for a mother, 25 for a father (e.g. mother of a 1755 child → `about 1734`). Chris doesn't know the real date; it's just a search seed. |
| Birthplace | **blank** |
